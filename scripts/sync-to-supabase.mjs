...
/**
 * Sync script to push new APIs from resources.json to Supabase
 *
 * This script:
 * 1. Reads db/resources.json
 * 2. Compares with existing APIs in Supabase (by documentation_link)
 * 3. Inserts new APIs with status='active'
 * 4. Does NOT update existing APIs (Supabase is source of truth for existing data)
 * 5. Does NOT delete from Supabase (deletions only happen in Supabase)
 *
...
/**
 * Fetch all existing API documentation links from Supabase
 */
async function fetchExistingApiLinks() {
  console.log("Fetching existing API links from Supabase...");
  const apis = await fetchAllPaginated("apis", "documentation_link");
  const links = new Set(apis.map((api) => api.documentation_link).filter(Boolean));
  console.log(`Found ${links.size} existing APIs in Supabase`);
  return links;
}
...
async function insertApis(apis) {
  ...
  console.log(`Inserting ${apis.length} new APIs...`);
  ...
  for (let i = 0; i < apis.length; i += batchSize) {
    ...
    const result = await supabaseRequest("apis", {
      method: "POST",
...
async function sync() {
  try {
    console.log("Starting resources.json → Supabase sync...\n");
    ...
    // Fetch existing API links
    const existingLinks = await fetchExistingApiLinks();

    // Find new APIs (not in Supabase)
    const newApis = [];
    const skippedNoLink = [];
    const skippedExists = [];
    ...
    console.log(`  - Already in Supabase: ${skippedExists.length}`);
    ...
    if (newApis.length > 0) {
      ...
      console.log(`\n✓ Successfully inserted ${inserted.length} new APIs`);
